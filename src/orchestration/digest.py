...
Produces the 'Last 24h: 12 new jobs, 7 passed filter, 3 in review
queue, est. cost $0.21' payload the dashboard banner consumes. The
digest is a *read-only* aggregation over:

* Plan run reports persisted by the orchestrator under
  ``data/plan_runs/<run_id>.json`` (rolled here so the digest
...
Persistence layout for plan run reports::

    data/
      plan_runs/
        20260516T230000Z-<run_id>.json   <- one file per run

Files are JSON dumps of :class:`PlanRunReport.to_dict()`. The
...
PLAN_RUNS_DIRNAME = "plan_runs"

# Filename: 20260516T230000Z-<run_id>.json
_PLAN_RUN_FILENAME_RE = re.compile(
    r"^(\d{8}T\d{6}Z)-([0-9a-fA-F-]+)\.json$"
)


def plan_runs_dir(root: Path | None = None) -> Path:
    """Resolve the directory where plan run reports live.

    Mirrors :func:`src.orchestration.plan_run.plan_run_pause_sentinel_path` --
    ...
    return base / "data" / PLAN_RUNS_DIRNAME
...
def persist_plan_run_report(
    report: PlanRunReport,
    *,
    root: Path | None = None,
) -> Path:
    """Write the report to ``data/plan_runs/<ts>-<run_id>.json``.
    ...
    PlanRunReport is immutable per run -- only re-fires of the same
...
def _parse_filename_ts(name: str) -> datetime | None:
    match = _PLAN_RUN_FILENAME_RE.match(name)
    if not match:
        return None
    try:
    ...
    *,
    ...
    for entry in sorted(target_dir.iterdir()):
        ...
        try:
            ...
            continue
        try:
            reports.append(PlanRunReport(**data))
        except TypeError:
            # Schema drift -- log and continue. Future-proofing for
            # when PlanRunReport gains fields.
            logger.warning(
...
def compute_digest(
    ...
    *,
    ...
    """Aggregate the last ``window_hours`` of plan run reports +
