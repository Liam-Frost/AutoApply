...
- **Active branch**: `feat/phase-17` (PR pending)
- **Current phase**: Phase 17 implementation complete -- 17.1 through 17.7 all landed + three rounds of codex-review fixes folded in (1 LinkedIn carry-over, 3 P1 + 6 P2 across the rounds: dict-to-RawJob coercion in scoring, review_queue persistence from the orchestrator, top_n bounds, JobPosting.id resolution for review entries, stale-row Approve button hidden, snapshot mismatch detection in pre-submit gate, refresh route now actually re-enqueues materials, LinkedIn description-only matches now get apply-target enrichment, review_queue UNIQUE narrowed to a partial-on-pending index).
- **Last verification**: 1530 passed, 1 skipped on `feat/phase-17`; `ruff check` clean; frontend build clean (vite 6.29s, 129kB gzip JS); alembic upgraded dev DB to revision `c9e1f3a7b8d4` (Phase 17.2 partial unique fix).
- **Blockers**: None
- **Next step**: Open PR `feat/phase-17 -> dev`, then start **Phase 18 (Multi-Tenancy & Auth Hardening)** on a fresh `feat/phase-18` branch. Phase 13.9 already landed the schema-level `tenant_id` so 18.1 / 18.3 / 18.5 / 18.6 are activations of existing work; 18.2 (FastAPI auth middleware), 18.4 (Redis namespace refactor), and 18.7 (per-tenant credential store) are net-new construction.
...
| Sub | Scope | Status |
|-----|-------|--------|
| 17.1 | **`nightly_run` orchestrator.** `src/orchestration/nightly_run.py` -- async `run_nightly(...)` use case, dependency-injected (search_fn / score_fn / enqueue_fn / pause_root / now). Flow: search via `use_job_index=True` (Phase 13.4 cache-first + freshness-driven refresh) → score_jobs (Phase 16-aware structured breakdowns) → top-N qualified → per job persist review_queue row + enqueue `materials.generate` + `application.prepare`. NEVER enqueues `application.submit` -- submission lives behind the Phase 17.5 pre-submit gate + human approval. Returns a JSON-serializable `NightlyRunReport` (`run_id`, status, started/finished/duration, total_jobs_seen, qualified, disqualified, borderline counter, selected, materials_task_ids, application_prepare_task_ids, review_entry_ids, errors, estimated_cost_usd, dry_run). Pause sentinel (17.7) short-circuits BEFORE search. Celery task wrapper `orchestration.nightly_run` + Beat entry at `crontab(hour=23)`. CLI `autoapply nightly run/enqueue/status` + `autoapply pause-nightly/resume-nightly`. | **Complete** (commits `771b6da`, `2d694e9`, `fe11907`) |
| 17.2 | **`review_queue` table + state machine.** Migration `b7d9a1e4f3c2` adds `review_queue(id, tenant_id, job_id, job_snapshot_id, run_id, materials_path, score_breakdown, company, title, status, decision, reason, created_at, reviewed_at, submitted_at, reviewer)`. `job_id` / `job_snapshot_id` intentionally NOT a FK so a row survives retention sweeps. CHECK-constrained status. Partial unique index on `(tenant, job, snapshot) WHERE status='pending'` (migration `c9e1f3a7b8d4`, codex round-3 fix -- previous full UNIQUE blocked the same snapshot from re-passing through the lifecycle in later nightly runs). State machine: `pending → approved → submitted` or `pending → rejected` or `pending → stale`; `approved → rejected | stale`; `stale → pending | rejected`; `submitted` / `rejected` terminal. `src/application/review.py` exposes `create_entry` (idempotent on pending) / `approve` / `reject` / `mark_submitted` / `mark_stale` / `refresh_stale` / `list_entries` / `bulk_approve` / `bulk_reject` / `bulk_reject_by_filter`. | **Complete** (commits `1fe3960`, `62c4314`) |
| 17.3 | **`/review` kanban UI** + API routes. `src/web/routes/review.py` exposes `GET /api/review[?status=]`, `GET /api/review/{id}`, single-item `POST .../approve`, `.../reject`, `.../refresh`, `.../submit`. Tenant-isolated (cross-tenant returns 404 not 403). Error mapping: `InvalidTransitionError → 409`, `LookupError → 404`. `frontend/src/views/ReviewQueueView.vue` -- four-column kanban (Pending / Approved / Submitted / Rejected). Stale rows surface in the Pending column with a Refresh button (Approve hidden for stale -- codex P2 fix). Approved column has primary Submit + secondary Reject. | **Complete** (commit `46e8834`) |
| 17.4 | **Bulk operations** at `POST /api/review/bulk/{approve,reject,reject-by-filter}`. Aggregate `{succeeded: [...], failed: [{id, error}]}` envelope so the UI renders "8 of 12 approved -- 4 failed: ...". Routes silently skip cross-tenant ids (so a guessed-id attack can't enumerate). Bulk-by-filter takes case-insensitive `company` / `keyword_in_title` substring predicates (ILIKE). Frontend exposes multi-select checkboxes + sticky bulk-action card + by-filter card. | **Complete** (commit `46e8834`) |
| 17.5 | **Pre-submit hard gate** `src/review/pre_submit_gate.py`. Runs `should_refresh(posting, "before_submit")` (6h budget) AND compares `entry.job_snapshot_id` vs `posting.latest_snapshot_id` (codex round-3 P1 fix -- catches the case where the posting was re-scraped after materials were generated against an older JD). Four `PreSubmitAction` values: `allow` (clear to submit), `refresh` (snapshot stale or snapshot mismatch -- flips entry to `stale`), `expired` (posting in expired/archived -- flips to `rejected`), `missing_binding` (entry not approved, no job_id, or posting row purged). Route `POST /api/review/{id}/submit` wraps: load → gate → on allow flip to submitted + enqueue `application.submit`; on block surface verdict with `ok=False`. | **Complete** (commits `4956f5c`, `fe11907`, `62c4314`) |
| 17.6 | **Morning digest at 08:00** `src/orchestration/digest.py`. Aggregates nightly_run reports persisted under `data/nightly_runs/<ts>-<run_id>.json` (filename-prefix windowing; gitignored) + live `count(*)` over review_queue grouped by status. Returns `DigestPayload(runs, total_jobs_seen, qualified, disqualified, borderline, selected, materials_enqueued, errors, paused_runs, estimated_cost_usd, review_queue_status, headline)`. Beat task `notifications.morning_digest` at `crontab(hour=8)`. `GET /api/digest?window_hours=N` (clamped to [1, 168]). Dashboard banner renders `headline` + per-status chips. | **Complete** (commit `b005fcb`) |
| 17.7 | **Kill switch.** `autoapply pause-nightly [--clear-pending]` + `autoapply resume-nightly`. The sentinel under `data/nightly_paused` is checked by `run_nightly` BEFORE search so a scheduled Beat tick short-circuits with `status="paused"`. `--clear-pending` is the vacation affordance: bulk-rejects every pending review_queue row with reviewer=`"pause-nightly"`, reason=`"paused for vacation"`. Approved / submitted / rejected / stale rows survive (a paused operator who already greenlit those still wants them when they return). Default behaviour (no flag) is purely the sentinel touch. | **Complete** (commit `208db10`) |
| **codex** | **Three rounds of codex-review fixes** folded in. Round 1: search jobs come back as serialized dicts but scorer needed RawJob (added `_coerce_job_to_rawjob`); orchestrator now persists review_queue rows directly (the `application.prepare` stub never would have populated them); `top_n <= 0` selects none, not all. Round 2: review entries were bound to `RawJob.id` instead of `JobPosting.id`, so the pre-submit gate always 404'd at submit time -- added `_resolve_and_patch_posting_ids` lookup; stale rows hid the broken Approve button. Round 3: pre-submit gate now compares snapshot ids; refresh route enqueues `jobs.enrich` + `materials.generate` so it actually refreshes; LinkedIn description-only matches no longer skip the redirect-enrichment pass; partial unique index for pending lets the same snapshot pass through lifecycle across runs. | **Complete** (commits `2d694e9`, `fe11907`, `62c4314`) |

**Verification**: 1530 passed, 1 skipped on `feat/phase-17`; `ruff check` clean; frontend build clean (vite 6.29s, 129kB gzip JS); alembic head at `c9e1f3a7b8d4`. The 'schedule at 23:00 Monday → wake Tuesday 08:00' integration test is deferred to a manual smoke since it requires the full Celery + Redis + LinkedIn stack; the unit + DB round-trip suites pin every individual contract.
