...
- **Active branch**: `feat/phase-15` (PR pending)
- **Current phase**: Phase 15 implementation complete -- 15.1 through 15.10 all landed + one round of codex-review P2 fixes folded in (legacy LaTeX manifest fallback; PDF page_count captured before close; asset subdirectories preserved on compile).
- **Last verification**: 1332 passed, 1 skipped on `feat/phase-15`; `ruff check` clean; alembic upgraded dev DB to revision `a3b9d52e7c41` (Phase 15.1 `source_resumes`); `codex review` returned no findings on second pass.
- **Blockers**: None
- **Next step**: Open PR `feat/phase-15 -> dev`, then start **Phase 16 (Filter Agent + Explainability)** on a fresh `feat/phase-16` branch. The first sub-phase (16.1) is the `RuleVerdict` schema evolution -- structured `rule_id` / `evidence_excerpt` / `job_snapshot_id` on every reject -- which is a prerequisite for the "Why was this filtered?" UI (16.3).
