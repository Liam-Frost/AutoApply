...
- **Active branch**: `feat/phase-14` (PR pending)
- **Current phase**: Phase 14 implementation complete -- 14.1 through 14.10 all landed + two rounds of codex-review fixes folded in (P1 cancel-revoke; P2 universal audit-row creation via `before_task_publish`; P2 cancelled-terminal preservation across all three lifecycle handlers).
- **Last verification**: 1161 passed, 1 skipped on `feat/phase-14`; `ruff check` clean; frontend build clean (vite 5.73s, 125kB gzip JS); alembic upgraded dev DB to revision `f2c5d83a91b6` (Phase 14.4); `codex review --uncommitted` returned no findings on second pass.
- **Blockers**: None
- **Next step**: Open PR `feat/phase-14 -> dev`, then start **Phase 15 (Resume & Cover Letter Generation v2)** on a fresh `feat/phase-15` branch. Phase 15.6 `jd_lookup` tool and Phase 15.7 `AgentCoverLetter` will be the first agent-driven tasks that exercise `AutoApplyTask.call_agent` end-to-end against a real broker.
