...
- **Phase 17** (Daily Run Loop + Review Queue: `nightly_run` orchestrator threads Phase 14 task queue + Phase 13 job index + Phase 12 cache + Phase 15/16 agents into a single 23:00-cron task that searches, scores with the 16-aware breakdowns, persists review_queue rows, and enqueues `materials.generate` + `application.prepare` for top-N -- never auto-submits; `review_queue` table with 5-state machine (`pending → approved → submitted | rejected | stale`); partial unique index on pending so the same snapshot can re-pass through the lifecycle; `/review` kanban with single-item + bulk approve/reject/refresh + submit; pre-submit hard gate that compares `entry.job_snapshot_id` vs `posting.latest_snapshot_id` and runs the 6h freshness budget; morning digest at 08:00 powered by per-run JSON reports under `data/nightly_runs/` + live queue counts; `autoapply pause-nightly [--clear-pending]` kill switch) — Complete

**1530 tests passing**, 1 skipped. `ruff` clean. Frontend builds clean. See [CHANGELOG](docs/CHANGELOG.md) and [AGENT_ARCHITECTURE.md](docs/AGENT_ARCHITECTURE.md) for details.
...
| 17 | Daily Run Loop + Review Queue — `nightly_run` orchestrator at 23:00 cron (search → score → persist review_queue + enqueue materials/prepare, never auto-submits), `review_queue` table + 5-state machine + partial unique on pending, `/review` kanban (single-item + bulk + filter), pre-submit hard gate (snapshot-id mismatch + 6h freshness budget), morning digest at 08:00 banner, `autoapply pause-nightly [--clear-pending]` — **Complete** | ~2 weeks |
